...
# ==================================================
st.set_page_config(
    page_title="Sandy’s Law — Square, Coupling & Collapse",
    layout="wide"
)
...
def compute_corner_dwell(Z, S, dt, th):
    dwell_steps = 0
    for z, s in zip(Z, S):
        if (z > th and s > th) or (z < 1 - th and s < 1 - th):
            dwell_steps += 1
    dwell_time = dwell_steps * dt
    dwell_frac = dwell_time / (len(Z) * dt)
...
# ---------- TOY 2: COUPLED (PERSISTENT SHEAR / LOOPS)
...
def corner_collapse_toy(steps, dt, drive_Z, drive_S, k_collapse, th):
    Z, S = 0.6, 0.4
    Zh, Sh = [], []
    ...
    for _ in range(steps):
        # SAME square driver as Toy 1
        dZ = drive_Z * (Z - 0.5)
        dS = drive_S * (S - 0.5)

        # Collapse: quench motion (stall), do NOT reverse
        if in_corner(Z, S):
            dZ *= (1 - k_collapse)
            dS *= (1 - k_collapse)
...
mode = st.sidebar.radio(
    "Toy Mode",
    ["Square (BPSR)", "Coupled (Shear / Loops)", "Square → Corner Collapse"]
)
...
k_zs = st.sidebar.slider("k_ZΣ (Σ → Z)", -3.0, 3.0, 1.6, 0.1)
k_sz = st.sidebar.slider("k_ΣZ (Z → Σ)", -3.0, 3.0, -1.4, 0.1)

drive_Z = st.sidebar.slider("Drive Z", 0.5, 2.0, 1.3, 0.1)
drive_S = st.sidebar.slider("Drive Σ", 0.5, 2.0, 1.3, 0.1)
k_collapse = st.sidebar.slider("Collapse Strength", 0.05, 0.9, 0.35, 0.05)
...
• **Collapse strength** → quenching of structure (stall), not reversal  
